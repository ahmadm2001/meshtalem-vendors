...
import { Save, ArrowRight, Plus, Trash2, PlusCircle, ChevronDown, ChevronUp, Settings2 } from 'lucide-react';
...
const slugify = (str: string) =>
  str.trim().toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_\u0590-\u05ff]/g, '').slice(0, 40) || `group_${Date.now()}`;
...
/** Normalise a raw option group from the API to the full interface */
const normaliseGroup = (g: any, idx: number): OptionGroup => ({
  id: g.id ?? `group_${idx}`,
  name: g.name ?? '',
  type: (g.type as OptionDisplayType) ?? 'single_radio',
  step: typeof g.step === 'number' ? g.step : idx + 1,
  required: typeof g.required === 'boolean' ? g.required : true,
  adminCategory: g.adminCategory ?? 'general',
  dependsOn: g.dependsOn,
  values: Array.isArray(g.values)
    ? g.values.map((v: any, vi: number) => ({
        id: v.id ?? `${g.id ?? `group_${idx}`}_val_${vi}`,
        label: v.label ?? '',
...
export default function EditProductPage() {
    ...
    productsApi.getById(id)
      .then((r) => {
        ...
        if (p.productOptions && Array.isArray(p.productOptions)) {
          const normalised = p.productOptions.map(normaliseGroup);
          setOptionGroups(normalised);
          // Expand all groups by default when editing
          setExpandedGroups(new Set(normalised.map((_: any, i: number) => i)));
        }
        if (p.categoryId) {
          categoriesApi.getAll().then((r2) => {
            const cats = r2.data || [];
  ...
  const toggleExpand = (idx: number) => {
    setExpandedGroups(prev => { const s = new Set(prev); s.has(idx) ? s.delete(idx) : s.add(idx); return s; });
  };

  const toggleAdvanced = (idx: number) => {
    setAdvancedGroups(prev => { const s = new Set(prev); s.has(idx) ? s.delete(idx) : s.add(idx); return s; });
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="card space-y-4">
            ...
            <div>
              ...
              <input name="nameAr" value={form.nameAr} onChange={handleChange} required className="input-field" dir="rtl" placeholder="לדוגמה: דלת כניסה פרמיום..." />
            </div>
            <div>
                  ...
                  <div key={group.id} className="border border-gray-200 rounded-xl bg-gray-50 overflow-hidden">
                    ...
                    <div className="flex items-center gap-2 p-3 bg-white border-b border-gray-100">
                      ...
                      <input
                        ...
                        dir="rtl"
                      />
                      <button type="button" onClick={() => toggleExpand(groupIdx)} className="p-1.5 text-gray-400 hover:text-gray-600 rounded-lg shrink-0">
                        {isExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                      </button>
                      <button type="button" onClick={() => removeOptionGroup(groupIdx)} className="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg shrink-0">
                      ...
                      <div className="p-3 space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <div>
                            ...
                            <select value={group.type} onChange={e => updateGroup(groupIdx, { type: e.target.value as OptionDisplayType })} className="input-field text-xs py-1.5">
                          ...
                          <div>
                            ...
                            <select value={group.adminCategory} onChange={e => updateGroup(groupIdx, { adminCategory: e.target.value })} className="input-field text-xs py-1.5">
                          ...
                          <div>
                            ...
                            <select value={group.required ? 'yes' : 'no'} onChange={e => updateGroup(groupIdx, { required: e.target.value === 'yes' })} className="input-field text-xs py-1.5">
                          ...
                          <div className="space-y-2">
                            {group.values.map((val, valueIdx) => (
                              <div key={val.id} className="flex items-center gap-2">
                                ...
                                <button type="button" onClick={() => removeOptionValue(groupIdx, valueIdx)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg shrink-0">
